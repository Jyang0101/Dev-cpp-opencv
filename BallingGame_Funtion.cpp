...
void barMove(void);//Bar移動控制函式
void scoreAcc(void);//分數統計函式 
void circleMove(void);//圓形向下移動控制函式
void gameInfo(void);//遊戲資訊顯示函式 
...
int main()
{
	...
	Image = cvCreateImage(ImageSize1,IPL_DEPTH_8U,3);//創造一個全黑的基本圖片,需要給它設定空間CvSize資料結構
   	cvNamedWindow("Circle_and_Bar",1);     	//繪圖視窗命名
	...
	do
	{
		...
		cvShowImage("Circle_and_Bar",Image);//顯示圖片 
